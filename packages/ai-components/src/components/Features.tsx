...
export function Features({ model = 'gpt-4o', prompt, stream = true, className, title, subtitle, count = 6, items }: FeaturesProps) {
          ...
          <div className={cn('py-16 px-4', className)}>
            ...
            <div className='max-w-7xl mx-auto'>
              <div className='text-center mb-12'>
                <h2 className='text-3xl font-bold'>{featuresTitle || (isStreaming ? 'Generating features...' : 'Key Features')}</h2>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
                {featureItems.length > 0 ? (
                  ...
                  <div className='col-span-full text-center text-muted-foreground'>No features available</div>
                )}
              </div>
            </div>
          </div>
