...
export function Testimonials({ model = 'gpt-4o', prompt, stream = true, className, title, count = 3, items }: TestimonialsProps) {
                  ...
                  testimonialItems.map((item: any, index: number) => (
                    <div key={index} className='border rounded-lg p-6 bg-card flex flex-col'>
                      ...
                      <div className='flex items-center mt-4'>
                        {item.avatar ? (
                          <img src={item.avatar} alt={item.author} className='w-12 h-12 rounded-full mr-4 object-cover' />
                        ) : (
