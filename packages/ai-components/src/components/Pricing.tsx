...
export function Pricing({ model = 'gpt-4o', prompt, stream = true, className, title, subtitle, tierCount = 3, tiers }: PricingProps) {
          ...
          <div className={cn('py-16 px-4', className)}>
            ...
            <div className='max-w-7xl mx-auto'>
              <div className='text-center mb-12'>
                <h2 className='text-3xl font-bold'>{pricingTitle || (isStreaming ? 'Generating pricing...' : 'Pricing Plans')}</h2>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
                {pricingTiers.length > 0 ? (
                  pricingTiers.map((tier: any, index: number) => (
                    <div key={index} className={cn('border rounded-lg p-8 bg-card flex flex-col', index === 1 && 'border-primary shadow-lg scale-105 z-10')}>
                      ...
                      <a
                        ...
                        className={cn(
                          ...
                          index === 1 ? 'text-white bg-primary hover:bg-primary/90' : 'text-primary bg-primary/10 hover:bg-primary/20'
                        )}
                      >
                        {tier.cta.text}
                      </a>
                  ...
                  <div className='col-span-full text-center text-muted-foreground'>No pricing tiers available</div>
                )}
              </div>
            </div>
          </div>
