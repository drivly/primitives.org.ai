...
export function alterSchemaForOpenAI(jsonSchema: JSONSchema7Definition): unknown {
  ...
  if (items) {
    result.items = Array.isArray(items) ? items.map(alterSchemaForOpenAI) : alterSchemaForOpenAI(items)
  }
  ...
  const illegalKeys = ['example', 'default', 'minimum', 'maximum', 'minLength', 'maxLength']
