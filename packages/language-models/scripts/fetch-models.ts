...
async function fetchModels() {
  try {
    ...
    fs.writeFileSync(path.join(fullDir, 'index.js'), `export const models = ${JSON5.stringify(models, null, 2)}`)

    fs.writeFileSync(path.join(__dirname, '../src/index.js'), `export const models = ${JSON5.stringify(modelSlugs, null, 2)}`)

    fs.writeFileSync(path.join(__dirname, '../src/types.ts'), typeDefinition)
