...
export const ChartComponent: React.FC<ChartComponentProps> = ({ chart, theme }) => {
  ...
  const renderBarChart = () => {
    ...
    return (
      <svg width='100%' height='200' viewBox={`0 0 ${width} ${height}`}>
        {/* Y-axis */}
        <line x1={padding} y1={padding} x2={padding} y2={height - padding} stroke={theme?.colorScheme === 'dark' ? '#444444' : '#dddddd'} strokeWidth='0.5' />
  ...
  const renderPieChart = () => {
    ...
    const slices = chartData.datasets[0].data.map((value, index) => {
      ...
      const slice = {
        path: pathData,
        fill: Array.isArray(chartData.datasets[0].backgroundColor) ? chartData.datasets[0].backgroundColor[index] : `hsl(${(index * 137) % 360}, 70%, 60%)`,
        startAngle,
        endAngle,
      }
    ...
    return (
      <svg width='100%' height='200' viewBox={`0 0 ${width} ${height}`}>
        {slices.map((slice, index) => (
          <path key={`slice-${index}`} d={slice.path} fill={slice.fill} stroke={theme?.colorScheme === 'dark' ? '#222222' : '#ffffff'} strokeWidth='0.5' />
        ))}
      </svg>
    )
  }
    ...
    return (
      <svg width='100%' height='200' viewBox={`0 0 ${width} ${height}`}>
        ...
        {dataPoints.map((point, index) => (
          <circle key={`point-${index}`} cx={point.x} cy={point.y} r='1.5' fill={theme?.colorScheme === 'dark' ? '#ffffff' : '#000000'} />
        ))}
      </svg>
    )
  }
